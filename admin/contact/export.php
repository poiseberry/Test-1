...
$queryListing = "select * from " . $table['contact']." order by pkid asc";
...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D3', 'Enquiry')
        ->setCellValue('E3', 'Date & Time');
...
while ($results_array = $results->fetchRow()) {
    ...
    $objPHPExcel->getActiveSheet()
            ...
            ->setCellValue('B' . $key, $results_array['phone'])
            ->setCellValue('C' . $key, $results_array['email'])
            ->setCellValue('D' . $key, $results_array['enquiry'])
            ->setCellValue('E' . $key, $results_array['created_date']);



    $ws = $objPHPExcel->getActiveSheet();
    $ws->getCell('B' . $key)->setValueExplicit($results_array['phone'], PHPExcel_Cell_DataType::TYPE_STRING);
    $ws->getCell('E' . $key)->setValueExplicit($results_array['created_date'], PHPExcel_Cell_DataType::TYPE_STRING);
}
