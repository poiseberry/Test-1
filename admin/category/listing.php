...
if ($type == "get_listing_data") {
    ...
    echo json_encode(
        SSP::complex($_GET, $sql_details, $table['category'], 'pkid', $columns, null, $where)
    );
    //exit everything
    exit();
}
...
if ($_POST['method'] == "delete_listing") {
    ...
    $query=get_query_delete($table['category'],$pkid);
    $database->query($query);
    //tracking what the user is doing
    do_tracking($user_username, 'Delete Category');
