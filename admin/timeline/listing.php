...
if ($type == "get_listing_data") {
    ...
    $columns = array(
        ...
        array('db' => 'cat_pkid', 'dt' => 3, 'formatter' => function ($d, $row){
            global $table;
            $database = new database();
        ...
        array('db' => 'pkid', 'dt' => 5, 'formatter' => function ($d, $row) {
    ...
    echo json_encode(
        SSP::complex($_GET, $sql_details, $table['time'], 'pkid', $columns, null, $where)
    );
    //exit everything
    exit();
}
...
if ($_POST['method'] == "delete_listing") {
    ...
    $query=get_query_delete($table['time'],$pkid);
    $database->query($query);
    //tracking what the user is doing
    do_tracking($user_username, 'Delete Timeline');
