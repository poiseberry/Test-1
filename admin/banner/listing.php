...
if ($type == "get_listing_data") {
    ...
    $columns = array(
        ...
        array('db' => 'img_url', 'dt' => 1, 'formatter' => function ($d, $row) {
            return '<img src="../files/banner/'.$d.'" height="100px">';
        }),
        array('db' => 'sort_order', 'dt' => 2),
    ...
    echo json_encode(
        SSP::complex($_GET, $sql_details, $table['banner'], 'pkid', $columns, null, $where)
    );
    //exit everything
    exit();
}
...
if ($_POST['method'] == "delete_listing") {
    ...
    $query=get_query_delete($table['banner'],$pkid);
    $database->query($query);
    //tracking what the user is doing
    do_tracking($user_username, 'Delete Banner');
